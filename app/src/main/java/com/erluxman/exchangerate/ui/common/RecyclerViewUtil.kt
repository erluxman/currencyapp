 ...
 */
fun <T> RecyclerView.bind(items: List<T>, @LayoutRes singleLayout: Int = 0, lm: RecyclerView.LayoutManager = LinearLayoutManager(context), singleBind: (View.(item: T) -> Unit)): FastListAdapter<T> {
    layoutManager = lm
    return FastListAdapter(items.toMutableList(), this
